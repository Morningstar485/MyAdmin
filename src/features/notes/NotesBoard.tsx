...
export function NotesBoard({ workspace: workspaceProp }: { workspace?: string }) {
    ...
    const sensors = useSensors(
        useSensor(PointerSensor, {
            activationConstraint: {
                distance: 8, // Drag starts after 8px movment, preventing accidental drags on clicks
            },
        })
    );
    ...
    const handleDragStart = (event: DragStartEvent) => {
        const { active } = event;
        if (active.data.current && active.data.current.type === 'note') {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (active.data.current?.type === 'note' && over.data.current?.type === 'folder') {
            ...
            // Don't move if it's the same folder (logic handled by not showing current folder in list, but safe to check)
    ...
    return (
        <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            <div className="h-full flex flex-col pt-6 overflow-hidden bg-slate-950">
                <div className="px-8 shrink-0">
                    <PageHeader
                        title="My Notes"
                        description="Capture ideas and organize your thoughts."
                        stats={[
                            { label: 'Notes', value: totalNotesCount },
                            { label: 'Folders', value: totalFoldersCount }
                        ]}
                ...
                {/* Content Area */}
                <div className="flex-1 overflow-y-auto px-8 pb-20 custom-scrollbar p-1">
                    {isLoading ? (
